...
| 10 | `porthole forward` has no container to redirect to. Three facts share this code, and the message and `--json`'s `kind` are what say which: something on this machine is listening on the port and no container publishes it (`not_published_by_container`), nothing is listening on it at all (`nothing_listening`), or Docker could not be read, so porthole never found out (`docker_unreadable`). The first means the port cannot be forwarded as it stands; the second is usually a mistyped number or a service that was not started. |
